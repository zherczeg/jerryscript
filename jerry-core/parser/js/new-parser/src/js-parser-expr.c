...
parser_parse_class_literal (parser_context_t *context_p) /**< context */
{
  ...
  while (PARSER_TRUE)
  {
    ...
    if (context_p->token.type == LEXER_PROPERTY_GETTER || context_p->token.type == LEXER_PROPERTY_SETTER)
    {
      ...
      int is_getter = (context_p->token.type == LEXER_PROPERTY_GETTER);
      ...
      is_computed = PARSER_FALSE;
      if (context_p->token.type == LEXER_RIGHT_SQUARE)
      {
        is_computed = PARSER_TRUE;
      }
      ...
      is_static = (status_flags & PARSER_CLASS_STATIC_FUNCTION);
    ...
    if (!(status_flags & PARSER_CLASS_STATIC_FUNCTION) && context_p->token.type == LEXER_CLASS_CONSTRUCTOR)
    {
      ...
      {
        super_called = PARSER_TRUE;
      }

      parser_flush_cbc (context_p);
      constructor_status_flags = status_flags | PARSER_CLASS_CONSTRUCTOR;
      ...
      util_set_function_literal (literal_p, parser_parse_function (context_p, constructor_status_flags));
...
{
  (const uint8_t *) "prototype", 9, LEXER_STRING_LITERAL, PARSER_FALSE
};
...
parser_parse_class (parser_context_t *context_p, /**< context */
                    int is_statement) /**< PARSER_TRUE - if class is parsed as a statement
                                       *   PARSER_FALSE - otherwise (as an expression) */
{
  int create_class_env, is_strict;

  PARSER_ASSERT (context_p->token.type == LEXER_KEYW_CLASS);
  ...
  create_class_env = (context_p->token.type == LEXER_KEYW_EXTENDS
  ...
  is_strict = context_p->status_flags & PARSER_IS_STRICT;
                                  ...
                                  uint32_t status_flags) /**< function status flags */
{
  ...
  {
    if (!lexer_check_next_character (context_p, '('))
    {
      lexer_next_token (context_p);
 ...
 *
 * @return PARSER_TRUE - if an arrow function is found
...
parser_check_arrow_function (parser_context_t *context_p) /**< context */
{
  ...
  int is_arrow_function = PARSER_TRUE;
  ...
  while (PARSER_TRUE)
  {
    ...
    is_arrow_function = PARSER_FALSE;
    break;
  }
...
{
  int is_empty_head = PARSER_TRUE;

  if (context_p->token.lit_location.length > 0)
  {
    is_empty_head = PARSER_FALSE;
  ...
  switch (context_p->token.type)
  {
    ...
    case LEXER_LITERAL:
    {
      ...
      else if (context_p->token.lit_location.type == LEXER_NUMBER_LITERAL)
      {
        ...
        {
          PARSER_ASSERT (context_p->lit_object.index <= CBC_PUSH_NUMBER_BYTE_RANGE_END);
    ...
    {
      parser_parse_class (context_p, PARSER_FALSE);
      return;
    }
#endif /* !CONFIG_DISABLE_ES2015_CLASS */
    ...
    {
      if ((lexer_check_next_character (context_p, '.')
            || lexer_check_next_character (context_p, '['))
          && (context_p->status_flags & PARSER_CLASS_HAS_SUPER))
      {
        uint32_t is_static;
        ...
        is_static = context_p->status_flags & PARSER_CLASS_STATIC_FUNCTION;
