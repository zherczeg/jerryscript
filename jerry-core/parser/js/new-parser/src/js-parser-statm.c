...
parser_parse_super_class_context_start (parser_context_t *context_p) /**< context */
{
  ...
  PARSER_ASSERT (context_p->token.type == LEXER_KEYW_EXTENDS
  ...
  {
    PARSER_ASSERT (context_p->status_flags & PARSER_CLASS_HAS_SUPER);
...
parser_parse_statements (parser_context_t *context_p) /**< context */
{
         ...
         || context_p->stack_top_uint8 != PARSER_STATEMENT_START)
  {
    ...
    switch (context_p->token.type)
    {
      ...
      {
        parser_parse_class (context_p, PARSER_TRUE);
        continue;
      }
#endif /* !CONFIG_DISABLE_ES2015_CLASS */
      ...
      case LEXER_KEYW_RETURN:
      {
        ...
        int return_with_literal = (context_p->last_cbc_opcode == CBC_PUSH_LITERAL);
#ifndef CONFIG_DISABLE_ES2015_CLASS
