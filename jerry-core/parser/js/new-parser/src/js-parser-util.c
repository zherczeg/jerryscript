...
parser_flush_cbc (parser_context_t *context_p) /**< context */
{
  ...
  if (context_p->is_show_opcodes)
  {
    ...
    if (flags & CBC_HAS_BYTE_ARG)
    {
      ...
      {
        printf (" number:%d", (int) context_p->last_cbc.value + 1);
      }
                             ...
                             int is_negative_number) /**< sign is negative */
{
  ...
  if (lit_value == UINT16_MAX)
  {
    ...
    PARSER_ASSERT (CBC_STACK_ADJUST_VALUE (PARSER_GET_FLAGS (opcode)) == 1);
  }
  else
  {
    ...
    PARSER_ASSERT (CBC_STACK_ADJUST_VALUE (PARSER_GET_FLAGS (opcode)) == 2);

    context_p->last_cbc.literal_index = lit_value;
  }

  PARSER_ASSERT (value > 0 && value <= CBC_PUSH_NUMBER_BYTE_RANGE_END);
